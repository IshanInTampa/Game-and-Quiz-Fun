(() => {
  ...
  function confettiColor() {
    return randChoice([
      "#ff5050", "#ffbe28", "#32c878", "#50a0ff", "#be8cff", "#ff78dc"
    ]);
  }
  ...
  function bindHoldButton(id, field) {
    const el = document.getElementById(id);
    const down = (e) => { touch[field] = true; ensureAudio(); e.preventDefault(); };
    const up   = (e) => { touch[field] = false; e.preventDefault(); };
  ...
  class Player {
    ...
    update(platforms, nowMs) {
      ...
      // jump input
      const jumpPressed = keys.has(" ") || keys.has("w") || keys.has("arrowup") || touch.jump;
  ...
  class Game {
    constructor(canvas) {
      ...
      this.mode = "start"; // start, play, question, feedback, win, game_over
      ...
      // Key actions that depend on mode
      window.addEventListener("keydown", (e) => {
        const k = e.key.toLowerCase();
        if (k === "escape") {
          // can't close tab; just reset to start
          this.mode = "start";
        }
    ...
    drawWorld() {
      ...
      roundRect(ctx, pr.x - 5, pr.y - 5, pr.w + 10, pr.h + 10, 12, false, true, "#be8cff", 3);
    ...
    drawHud() {
      ...
      ctx.font = (now < this.coinPopUntil) ? "bold 26px Arial" : "20px Arial";
      ctx.fillText(
      ...
      ctx.fillText("Move: A/D or ←/→   Jump: Space/W/↑   Touch portal for quiz   (Esc = back to start)", 16, 52);

      if (now < this.msgUntil && this.msg) {
        ctx.fillStyle = "#b42828";
        ctx.font = "bold 28px Arial";
        const w = ctx.measureText(this.msg).width;
        ctx.fillText(this.msg, (WIDTH - w)/2, 95);
      }
    }
    ...
    drawStart() {
      ...
      const lines = [
        ...
        "• Answer with keys 1 / 2 / 3 (mobile buttons supported)",
        `• You have ${MAX_LIVES} lives (wrong answers reduce lives)`,
        "",
        "Press ENTER to start   |   Press ESC to return here anytime"
      ];
      let y = 175;
    ...
    drawQuestion() {
      ...
      // overlay card
      const card = rect(80, 80, WIDTH - 160, HEIGHT - 160);
      roundRect(ctx, card.x, card.y, card.w, card.h, 18, true, true, OVERLAY_BG, 2, "#d2d2d2");
      ...
      ctx.fillText(title, card.x + 20, card.y + 45);
      ...
      ctx.fillStyle = "#555";
      ctx.font = "16px Arial";
      ctx.fillText("Press 1 / 2 / 3 to answer", card.x + 20, card.y + card.h - 22);

      // clickable answer buttons (mobile-friendly)
      const btnY = card.y + card.h - 78;
      const btnW = 160, btnH = 44, gap = 16;
      const startX = card.x + 20;
      this.answerButtons = [0,1,2].map(i => rect(startX + i*(btnW+gap), btnY, btnW, btnH));
      for (let i = 0; i < 3; i++) {
        const b = this.answerButtons[i];
        roundRect(ctx, b.x, b.y, b.w, b.h, 12, true, true, "#f3f5f9", 1, "#cfd6e3");
        ctx.fillStyle = "#202530";
        ctx.font = "bold 18px Arial";
        const label = `${i+1}) Choose`;
        ctx.fillText(label, b.x + (b.w - ctx.measureText(label).width)/2, b.y + 28);
      }
    }
    ...
    drawWin() {
      ...
      ctx.font = "16px Arial";
      const h = "Refresh page to play again (or press ESC to return to start).";
    ...
    drawGameOver() {
      ...
      ctx.font = "16px Arial";
      const h = "Press R to restart, or ESC to return to start.";
      ctx.fillText(h, (WIDTH - ctx.measureText(h).width)/2, 292);
    }
    ...
    render() {
      ...
      if (this.mode === "play") return this.drawWorld();
      if (this.mode === "question") return this.drawQuestion();
  ...
  // Mouse/touch click handling (answer buttons)
  canvas.addEventListener("pointerdown", (e) => {
    ensureAudio();
    const rectB = canvas.getBoundingClientRect();
    const mx = (e.clientX - rectB.left) * (canvas.width / rectB.width);
    const my = (e.clientY - rectB.top) * (canvas.height / rectB.height);

    if (game.mode === "question" && game.answerButtons) {
      for (let i = 0; i < game.answerButtons.length; i++) {
        const b = game.answerButtons[i];
